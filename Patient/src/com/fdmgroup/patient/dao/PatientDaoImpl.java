...
import com.fdmgroup.patient.utils.JDBCUtils;
...
public class PatientDaoImpl implements PatientDao {
	...
	@Override
	public void insertTodo(Patient todo) throws SQLException {
	...
	@Override
	public Patient selectTodo(long todoId) {
		Patient todo = null;
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			while (rs.next()) {
				...
				todo = new Patient(id, title, username, description, targetDate, isDone);
			}
		} catch (SQLException exception) {
			JDBCUtils.printSQLException(exception);
		}
	...
	@Override
	public List<Patient> selectAllTodos() {

		// using try-with-resources to avoid closing resources (boiler plate code)
		List<Patient> todos = new ArrayList<>();
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			while (rs.next()) {
				...
				LocalDate targetDate = rs.getDate("target_date").toLocalDate();
				boolean isDone = rs.getBoolean("is_done");
				todos.add(new Patient(id, title, username, description, targetDate, isDone));
			}
		} catch (SQLException exception) {
			JDBCUtils.printSQLException(exception);
		}
	...
	@Override
	public boolean updateTodo(Patient todo) throws SQLException {
		boolean rowUpdated;
		try (Connection connection = JDBCUtils.getConnection();
